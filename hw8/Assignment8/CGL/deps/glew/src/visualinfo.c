...
int main(int argc, char** argv) {
  ...
  if (!displaystdout) {
    ...
    if (fopen_s(&file, "visualinfo.txt", "w") != 0) file = stdout;
#else
...
void VisualInfoARB(GLContext* ctx) {
  ...
  if (!verbose) {
    ...
    fprintf(file,
            ...
            "--------------+----------+\n");
    fprintf(file,
            " |     |          visual         |      color      | ax dp st |   "
            "   accum      |   layer  |\n");
    fprintf(file,
            " |  id | tp ac gd fm db sw st ms |  sz  r  g  b  a | bf th cl |  "
            "sz  r  g  b  a | ov un sw |\n");
    fprintf(file,
            " +-----+-------------------------+-----------------+----------+---"
            "--------------+----------+\n");
...
void VisualInfoGDI(GLContext* ctx) {
  ...
  if (!verbose) {
    fprintf(file,
            ...
            "-----------\n");
    fprintf(file,
            "   visual   x  bf  lv rg d st ge ge  r  g  b a  ax dp st   accum "
            "buffs    ms \n");
    fprintf(file,
            " id  dep tp sp sz  l  ci b ro ne ac sz sz sz sz bf th cl  sz  r  "
            "g  b  a ns b\n");
    fprintf(file,
            "------------------------------------------------------------------"
            "-----------\n");
  ...
  } else /* verbose */
  {
    ...
    for (i = 1; i <= maxpf; i++) {
      ...
      fprintf(
          file, "    generic=%d generic accelerated=%d\n",
  ...
  if (fbc) {
    if (!verbose) {
      ...
      fprintf(file,
              ...
              "   accum    |   ms  |  cav |\n");
      fprintf(file,
              " |  id | tp xr cl fm db st lv xp |  sz  r  g  b  a | bf th cl | "
              "r  g  b  a  | ns  b |  eat |\n");
      fprintf(file,
              " +-----+-------------------------+-----------------+----------+-"
              "------------+-------+------+\n");
